...
/**
 * Scroll vers le bas des messages pour div messages
 */
...
socket.on('chat-message', function (message) {
  ...
  $('#messages').append($('<li>').html(_time + '<span class="username">' + message.username + '</span> '+ message.text));
  scrollToBottom();
});
...
socket.on('service-message', function (message) {
  var _time = TimestampAMPM();
  $('#messages').append($('<li class="' + message.type + '">').html(_time + '<span class="info">information</span> ' + message.text));
  scrollToBottom();
}); 
